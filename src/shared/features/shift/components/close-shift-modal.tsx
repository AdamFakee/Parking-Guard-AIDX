...
const DISCREPANCY_REASONS = [
  'Trả nhầm tiền cho khách',
  'Khách nợ chưa trả',
  'Nhập sai số tiền mặt',
...
export const CloseShiftModal = forwardRef<CloseShiftModalRef>((_, ref) => {
  ...
  const { control, watch, handleSubmit, reset, setValue } = useForm<LogoutForm>({
                      ...
                      <View>
                        ...
                        <View className="flex-row flex-wrap gap-2 mb-3">
                          {DISCREPANCY_REASONS.map((r) => {
                            const isSelected = watch('reason') === r;
                            return (
                              <Pressable 
                                key={r}
                              ...
                              >
                                <Text className={`text-xs ${isSelected ? 'text-red-700 font-bold' : 'text-slate-600'}`}>
                                  {r}
                                </Text>
                              </Pressable>
                            );
                          ...
                          <Pressable 
                            onPress={() => setValue('reason', '')}
                            className={`px-3 py-2 rounded-lg border ${
                              !DISCREPANCY_REASONS.includes(watch('reason') || '') && watch('reason') !== ''
                                ? 'bg-red-50 border-red-200' 
                                : 'bg-white border-slate-200'
                            }`}
                          >
                            <Text className={`text-xs ${!DISCREPANCY_REASONS.includes(watch('reason') || '') && watch('reason') !== '' ? 'text-red-700 font-bold' : 'text-slate-600'}`}>
                              Lý do khác...
                            </Text>
                          </Pressable>
                        </View>
                        ...
                        {(!DISCREPANCY_REASONS.includes(watch('reason') || '') || watch('reason') === '') && (
                          <ControlledInput
                            ...
                            name="reason"
                            multiline
                            placeholder="Mô tả cụ thể lý do chênh lệch..."
